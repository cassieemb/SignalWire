...
answers = dict()
client = signalwire_client(os.getenv('SIGNALWIRE_PROJECT'), os.getenv('SIGNALWIRE_TOKEN'), signalwire_space_url = os.getenv('SIGNALWIRE_SPACE'))
...
def send_link():
    ...
    print(message.sid)
    response = VoiceResponse()
    response.say("Thank you for calling. An SMS has been sent and you may now complete the questionnaire at your convenience via web browser."
                 "Goodbye!")
    response.hangup()
    return toXML(response)
...
def question_two():
    ...
    with response.gather(num_digits=8, action=url_for('question_three'), method="POST") as g:
        g.say('Thank you. What was your expected arrival date? Please enter the date in the order of year, month, and day '
...
def end_questionnaire():
    ...
    response = VoiceResponse()
    response.say('Thank you for answering these questions. Your responses will be sent to our office. Have a great day!')
