...
for session in roomSessions:
    d.append((session['id'], session['name'], session['start_time'], session['end_time'], session['duration'],
...
print(
    f"There were {len(df)} total room sessions with a total duration of {round((df['Duration (seconds)'].sum()) / 60, 2)} minutes and a total cost of {'${:,.2f}'.format(df['Cost in Dollars'].sum())} dollars.")
